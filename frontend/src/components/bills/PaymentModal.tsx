...
import type { Bill, BankAccount } from '../../types';
...
export function PaymentModal({ isOpen, onClose, onSuccess, bill, companyId }: PaymentModalProps) {
  const [bankAccounts, setBankAccounts] = useState<BankAccount[]>([]);
  const [loading, setLoading] = useState(true);

  const balanceAmount = bill.total - bill.paidAmount;
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold">MUR {bill.total.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-gray-600">Amount Paid</p>
              <p className="font-semibold">MUR {bill.paidAmount.toFixed(2)}</p>
            </div>
            <div className="col-span-2">
              <p className="text-gray-600">Balance Due</p>
              <p className="text-xl font-bold text-orange-600">MUR {balanceAmount.toFixed(2)}</p>
            </div>
          </div>
        </div>
